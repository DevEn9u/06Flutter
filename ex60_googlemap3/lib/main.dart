...
class _MyHomePageState extends State<MyHomePage> {
  ...
  // 위도, 경도 계속 구하기
  void getCurrentLocatrion() async {
    await Permission.location.request().then(
      (status) {
        if (status == PermissionStatus.granted) {
          // 거리가 10m 이상 변해야 Listener에 위치가 전달된다.
  ...
  void newPosition(Position position) async {
    ...
    // Marker의 위치도 새로운 위치로 옮긴다.
    markerAdd();
  }

  late BitmapDescriptor customMarker;
  Future<void> setCustomerMarker() async {
